...
{
    // 复制IP头
    ip_hdr_t* ip_hdr = (ip_hdr_t*)malloc(sizeof(ip_hdr_t));
    ip_hdr_t* ip_ptr = (ip_hdr_t*)(buf->data - sizeof(ip_hdr_t));
    memcpy(ip_hdr, ip_ptr, sizeof(ip_hdr_t));
    ...
    // 拷贝IP头部
    memcpy(ip_ptr, ip_hdr, sizeof(ip_hdr_t));
    free(ip_hdr);
...
void udp_in(buf_t *buf, uint8_t *src_ip)
{
    ...
    hdr->checksum16 = 0;
    if (checksum != udp_checksum(buf, src_ip, net_if_ip)) return;
